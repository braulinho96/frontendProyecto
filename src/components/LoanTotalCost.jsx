...
const LoanTotalCost = () => {
  ...
  const calculateLoanCosts = async (loan) => {
    try {
      ...
      const monthlyPayment = await LoanService.calculateMonthlyPayment(
        financing_amount,
        interest_rate,
        term
      );
  ...
  const handleAccept = async () => {
    try {
      await LoanService.updateLoan({ ...loan, solicitudeState: 'E5' });
  ...
  const handleReject = async () => {
    try {
      await LoanService.updateLoan({ ...loan, solicitudeState: 'E8' });
